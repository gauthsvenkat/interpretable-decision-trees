...
class DQNPolicy:
    def __init__(self, env, model_path):
        ...
        with self.graph.as_default():
            with tf.compat.v1.variable_scope('deepq'):
                # Observations
                self.imgs = tf.compat.v1.placeholder(tf.uint8, [None] + list(self.input_shape), name='observation')

                # Randomness
                self.stochastic_ph = tf.compat.v1.placeholder(tf.bool, (), name='stochastic')
                self.update_eps_ph = tf.compat.v1.placeholder(tf.float32, (), name='update_eps')
                eps = tf.compat.v1.get_variable('eps', (), initializer=tf.compat.v1.constant_initializer(0))

                # Q-function
                with tf.compat.v1.variable_scope('q_func'):
                    # Normalization
                ...
                deterministic_actions = tf.argmax(input=self.qs, axis=1)

                # Stochastic actions
                batch_size = tf.shape(input=self.imgs)[0]
                ...
                # Output actions
                self.output_actions = tf.cond(pred=self.stochastic_ph, true_fn=lambda: stochastic_actions, false_fn=lambda: deterministic_actions)
                self.update_eps_expr = eps.assign(tf.cond(pred=self.update_eps_ph >= 0, true_fn=lambda: self.update_eps_ph, false_fn=lambda: eps))

            # Load
            tf.compat.v1.train.Saver().restore(self.sess, self.model_path)
