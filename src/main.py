...
def main(args):
    ...
    student = viper.get_student(env, oracle, train=args.train, save_path_specifier=args.student_path,depth=args.max_depth,optimal_tree=args.optimal_tree,cp=args.cp)
    bc = behavioralCloning.get_student(env, oracle, train=args.train, save_path_specifier=args.bc_path,depth=args.max_depth,optimal_tree=args.optimal_tree,cp=args.cp)
    ...
    e = Evaluate(env, oracle, [student, bc, simple], n_rollouts=args.rollouts, policy_names=["Student", "Bc"], experiment = args.experiment, no_print=args.no_print, optimal = args.optimal_tree, cp=args.cp)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train',type=bool, default=False, help='Set to true to train')
    ...
    parser.add_argument('--rollouts', type=int, default=100, help='How many rollouts to use')
    ...
    parser.add_argument('--cp', type=float, default=0, help='The cp for the generated optimal tree. Choose from [0, 0.5, 1]')
